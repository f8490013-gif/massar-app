...
import { listenTrip, updateTrip, listenDriverLocation } from '../../firebase/db'
...
export default function TrackRide() {
  const { activeRide, setActiveRide, showNotification, isFirebase } = useApp()
  ...
  // Firestore trip state
  const [tripData, setTripData]   = useState(null)
  const [driverInfo, setDriverInfo] = useState(ride.driver)

  // Driver position animation
  const [progress, setProgress]   = useState(0)
  const [steps, setSteps]         = useState(STEPS)
  const [arrived, setArrived]     = useState(false)
  const [realDriverPos, setRealDriverPos] = useState(null)

  // Listen to Firestore trip document
  useEffect(() => {
    if (!isFirebase || !ride.id) return
    const unsub = listenTrip(ride.id, data => {
      setTripData(data)
      if (data.driverName) {
        setDriverInfo(prev => ({
          ...prev,
          name:   data.driverName,
          rating: data.driverRating ?? prev?.rating,
          car:    data.driverCar    ?? prev?.car,
          plate:  data.driverPlate  ?? prev?.plate,
  ...
  // Listen to driver GPS location if we know the driver
  useEffect(() => {
    const dId = tripData?.driverId
    if (!isFirebase || !dId) return
    const unsub = listenDriverLocation(dId, loc => {
      setRealDriverPos([loc.lat, loc.lng])
    })
    return unsub
  }, [isFirebase, tripData?.driverId])
    ...
    const id = setInterval(() => {
      setProgress(p => {
        ...
        if (next >= 1) {
          ...
          // Update Firestore trip to completed
          if (isFirebase && ride.id) {
            updateTrip(ride.id, { status: 'completed' }).catch(() => {})
          }
          clearInterval(id)
          return 1
        }
  ...
  // Animated driver position (simulated, overridden by real GPS if available)
  const simulatedDriverPos = (() => {
    if (progress < 0.3) return lerp(drvStart, origin, progress / 0.3)
    return lerp(origin, dest, (progress - 0.3) / 0.7)
  })()
  const driverPos = realDriverPos ?? simulatedDriverPos

  // ETA countdown
  const totalSecs = Math.round((1 - progress) * 480)
  ...
  async function cancelRide() {
    if (isFirebase && ride.id) {
      await updateTrip(ride.id, { status: 'cancelled' }).catch(() => {})
    }
    showNotification('تم إلغاء الرحلة', 'error')
    setActiveRide(null)
    navigate('/passenger')
  ...
  return (
    <Layout title="تتبع الرحلة">
      <div className="max-w-xl mx-auto space-y-5">
        ...
        <div className="card">
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <p className="font-bold text-lg">{driverInfo?.name ?? 'جارٍ التعيين…'}</p>
              <p className="text-sm text-gray-500 truncate">{driverInfo?.car ?? ''}</p>
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                {driverInfo?.rating && (
                  <span className="flex items-center gap-1 text-sm">
                    <Star size={13} className="text-primary-500 fill-primary-500" />
                    <strong>{driverInfo.rating}</strong>
                  </span>
                )}
                {driverInfo?.plate && (
        ...
        {/* Actions */}
        {arrived ? (
          <button onClick={() => { setActiveRide(null); navigate('/passenger') }}

    ...
    <Layout title="لوحة الإدارة">
      <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between mb-5">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                إجمالي {adminTripsChart.reduce((a, d) => a + d.trips, 0)} رحلة
              </p>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            { label: 'إضافة مدرسة',  icon: '🏫', color: 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400' },
            { label: 'إضافة سائق',   icon: '🚗', color: 'bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400' },
            { label: 'إنشاء تقرير',  icon: '📊', color: 'bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400' },
            { label: 'إرسال إشعار',  icon: '🔔', color: 'bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400' },
          ].map(({ label, icon, color }) => (
            <button key={label} className={clsx('card flex flex-col items-center gap-3 py-5 hover:shadow-md transition-shadow', color)}>
